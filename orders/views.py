from .models import CartItem, Dish, OrderedItem
...
def create_order(request):
    ...
    if cart_items:
        ...
        # Создайте соответствующие записи OrderedItem для каждого блюда в корзине
...
def view_orders(request):
    ...
    # Создайте список, в котором для каждого заказа будет информация о блюдах и их ценах.
    order_info_list = []
    for order in orders:
        order_info = {
            'order': order,
            'ordered_items': order.ordered_items.all(),  # Блюда в заказе
            'total_price': order.total_price,  # Общая цена заказа
        }
        order_info_list.append(order_info)

    return render(request, 'orders/view_orders.html', {'order_info_list': order_info_list})
